...
contract StarNotary is ERC721 {
    ...
    string public constant name = 'MADHURTOKEN';
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        ...
        if (owner1 != msg.sender && owner2 != msg.sender) {
            revert('Failed to exchange, sender does not own any of the tokens.');
        }
    ...
    function transferStar(address _to1, uint256 _tokenId) public {
        ...
        if (msg.sender != ownerOf(_tokenId))  {
            revert('Failed to transfer token, sender does not own the token.');
        }
